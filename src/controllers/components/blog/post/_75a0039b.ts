...
class Controller extends Base {
  ...
  protected async get(data: Input[]): Promise<{[Identifier: string]: HierarchicalDataTable}> {
    return new Promise(async (resolve, reject) => {
      try {
        const current = await DatabaseHelper.retrieve(RequestHelper.createInputs({
            'blog.path': this.request.params.path,
            'blog.gallery.bid': null,
            'blog.category.bid': null
        ...
        const all = await DatabaseHelper.retrieve(RequestHelper.createInputs({
            'blog.active': true
          }), ProjectConfigurationHelper.getDataSchema().tables['blog'],
          this.request.session,   // session variables
          false,                  // real-time updates
        ...
        await DatabaseHelper.update(RequestHelper.createInputs({
            'blog.path': this.request.params.path,
            'blog.view': current['blog'].rows[0].columns.view + 1
          }), ProjectConfigurationHelper.getDataSchema().tables['blog'],
        ...
        for (const blog of current.blog.rows) {
          for (const row of blog.relations.category.rows) {
            row.columns.link = '/blog/categories/' + row.columns.path;
          }
        }
  ...
  protected initialize(request: Request): [ActionType, DataTableSchema, Input[]] {
    ...
    RequestHelper.registerInput('7005d090', undefined, undefined, undefined);
    ValidationHelper.registerInput('7005d090', "Textbox 1", false, undefined, undefined, null);
    for (let input of RequestHelper.getInputs(this.pageId, request, '7005d090')) {
